...
## Upload-only replay (no live API call)

Replay a previously-captured `output/*.json` snapshot into an instance's inbox
without hitting the Falcon API — useful for demos or replaying a known-good
capture:

```bash
# Auto-pick the most recently modified file under output/
npx tsx collect.ts <instance> --upload-recent

# Or replay a specific file
npx tsx collect.ts <instance> --upload-recent output/crowdstrike-<timestamp>.json
```
...
See `windmill/templates/crowdstrike-collector.ts` in the `sightline-prism-windmill` repository for the Windmill deployment template. [Install Windmill](/docs/prism/install/windmill) covers the setup.
